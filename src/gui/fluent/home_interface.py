...
class HomeInterface(ScrollArea):
    ...
    def _populate_engines(self):
        ...
        engines = [
            ...
            (TranslationEngine.OPENAI, self.config_manager.get_ui_text("translation_engines.openai", "🤖 OpenAI / OpenRouter")),
            (TranslationEngine.GEMINI, self.config_manager.get_ui_text("translation_engines.gemini", "✨ Google Gemini")),
            (TranslationEngine.LOCAL_LLM, self.config_manager.get_ui_text("translation_engines.local_llm", "🖥️ Local LLM (Ollama/LM Studio...)")),
        ]
    ...
    def _on_engine_changed(self):
        ...
        # Show warning for censored AI models
        if engine in [TranslationEngine.OPENAI, TranslationEngine.GEMINI]:
            # Use InfoBar for non-blocking warning
        ...
        try:
            if engine == TranslationEngine.OPENAI:
                self.translation_manager.add_translator(
                    ...
                    OpenAITranslator(
                        api_key=self.config_manager.api_keys.openai_api_key,
                        model=self.config_manager.translation_settings.openai_model or "gpt-3.5-turbo",
                        base_url=self.config_manager.translation_settings.openai_base_url,
                ...
                # Automatically suggest uncensored models if using OpenRouter
                if "openrouter" in (self.config_manager.translation_settings.openai_base_url or ""):
                     self._add_log("info", self.config_manager.get_ui_text("log_openrouter_hint", "💡 Tip: OpenRouter detected. Try uncensored models like 'dolphin-mistral'."))

            elif engine == TranslationEngine.GEMINI:
                # Add warning for Gemini content filter
                if self.config_manager.translation_settings.gemini_safety_settings != "BLOCK_NONE":
                    self._add_log("warning", self.config_manager.get_ui_text("log_gemini_safety_warning", "⚠️ Gemini Safety Filter Active. Set to BLOCK_NONE recommended."))
                
                # Set fallback to Google Web since Gemini filters often
                gemini_translator = GeminiTranslator(
                    api_key=self.config_manager.api_keys.gemini_api_key,
                    model=self.config_manager.translation_settings.gemini_model or "gemini-pro",
                    safety_level=self.config_manager.translation_settings.gemini_safety_settings,
                    proxy_manager=self.proxy_manager if use_proxy else None,
                    config_manager=self.config_manager,
                    temperature=self.config_manager.translation_settings.ai_temperature,
                    timeout=self.config_manager.translation_settings.ai_timeout,
                ...
                # Add fallback
                fallback_translator = GoogleTranslator(self.proxy_manager, self.config_manager)
                gemini_translator.set_fallback_translator(fallback_translator)
                ...
                self.translation_manager.add_translator(TranslationEngine.GEMINI, gemini_translator)
                 ...
                 self.translation_manager.add_translator(
                    ...
                    DeepLTranslator(
                        api_key=self.config_manager.api_keys.deepl_api_key,
                        proxy_manager=self.proxy_manager
                    )
                 )
        except ImportError as ie:
             self._show_warning(
                 self.config_manager.get_ui_text("error_library_missing", "Library Error"), 
                 self.config_manager.get_ui_text("error_library_missing_detail", f"Missing library: {ie}").format(error=ie)
             )
             return
        except Exception as e:
             self._show_warning(
                 self.config_manager.get_ui_text("error_init_failed", "Initialization Error"), 
                 self.config_manager.get_ui_text("error_init_failed_detail", f"Engine init failed: {e}").format(error=e)
             )
             return
