...
class MainWindow(QMainWindow):
    ...
    def create_control_panel(self) -> QWidget:
        ...
        self.directory_input.setPlaceholderText("Oyun EXE dosyasını seçin (örn: Game.exe)")
        ...
        input_layout.addRow("Oyun EXE:", dir_layout)
    ...
    def browse_game_exe(self):
        ...
        if file_path:
            ...
            self.log_text.clear()
            self._add_log("info", f"EXE seçildi: {file_path}")
            ...
            if os.path.isdir(game_dir):
                self._add_log("info", "✓ Geçerli Ren'Py projesi bulundu")
                ...
                if has_rpy:
                    self._add_log("info", "✓ .rpy dosyaları mevcut")
                elif has_rpyc:
                    self._add_log("warning", "⚠ Sadece .rpyc dosyaları var - decompile gerekli")
            else:
                self._add_log("error", "✗ 'game' klasörü bulunamadı")
    ...
    def start_integrated_translation(self):
        ...
        self._add_log("info", "🚀 Pipeline başlatılıyor...")
    ...
    def stop_integrated_translation(self):
        ...
        if hasattr(self, 'pipeline') and self.pipeline:
            self._add_log("warning", "⏹ Durdurma isteği gönderildi...")
            self.pipeline.stop()
    ...
    def _on_progress_updated(self, current: int, total: int, text: str):
        ...
        if current % 10 == 0 or current == total:
            self._add_log("info", f"Çevriliyor: {current}/{total}")
    ...
    def _on_pipeline_finished(self, result):
        ...
        if result.success:
            ...
            if result.stats:
                stats = result.stats
                self._add_log("info", f"📊 Toplam: {stats['total']} giriş")
                self._add_log("info", f"✓ Çevrilmiş: {stats['translated']}")
                self._add_log("info", f"○ Çevrilmemiş: {stats['untranslated']}")
            
            if result.output_path:
                self._add_log("info", f"📁 Çıktı: {result.output_path}")
            ...
            if result.error:
                self._add_log("error", f"Detay: {result.error}")
