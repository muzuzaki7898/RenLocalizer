...
class TranslationPipeline(QObject):
    ...
    def _run_pipeline(self) -> PipelineResult:
        ...
        # 1. Doğrulama
        self._set_stage(PipelineStage.VALIDATING, "Proje doğrulanıyor...")
        ...
        if not has_rpy and has_rpyc and self.auto_unren:
            self._set_stage(PipelineStage.UNREN, "Decompile ediliyor (UnRen)...")
        ...
        self._set_stage(PipelineStage.GENERATING, "Kaynak dosyalar hazırlanıyor...")
        ...
        # 3. Translate komutu
        self._set_stage(PipelineStage.GENERATING, f"Çeviri dosyaları oluşturuluyor ({self.target_language})...")
        ...
        # 4. Parse
        self._set_stage(PipelineStage.PARSING, "Çeviri dosyaları okunuyor...")
        ...
        if not tl_files:
            return PipelineResult(
                success=False,
                message="Çeviri dosyaları bulunamadı veya parse edilemedi",
                stage=PipelineStage.ERROR
            )
        ...
        # 6. Kaydetme
        self._set_stage(PipelineStage.SAVING, "Çeviriler kaydediliyor...")
        ...
        self._set_stage(PipelineStage.COMPLETED, "Tamamlandı!")
        
        return PipelineResult(
            success=True,
            message=f"{len(translations)} metin çevrildi, {saved_count} dosya kaydedildi",
            stage=PipelineStage.COMPLETED,
            stats=stats,
            output_path=tl_dir
        )
