...
class TranslationPipeline(QObject):
    ...
    def _run_pipeline(self) -> PipelineResult:
        ...
        # .rpymc dosyalarını bul
        from src.core.rpymc_reader import extract_rpymc_ast
        rpymc_files = self._find_rpymc_files(game_dir)
        rpymc_strings = []
        for rpymc_path in rpymc_files:
            try:
                ast_root = extract_rpymc_ast(rpymc_path)
                rpymc_strings.extend(self._extract_strings_from_rpymc_ast(ast_root))
            except Exception as e:
                self.log_message.emit('warning', f".rpymc extraction failed: {rpymc_path} ({e})")

        # rpymc_strings artık mevcut çeviri listesine eklenebilir (örnek: self.rpymc_strings = rpymc_strings)
        self.rpymc_strings = rpymc_strings
        
        # DEBUG: .rpymc string sayısını logla
        self.log_message.emit('info', f"[DEBUG] .rpymc string count: {len(self.rpymc_strings)}")
        ...
        # --- .rpymc stringlerini source_texts ve all_entries'ye ekle ---
        if hasattr(self, 'rpymc_strings') and self.rpymc_strings:
            from src.core.tl_parser import TranslationEntry
            self.log_message.emit('info', f"[DEBUG] .rpymc stringlerini source_texts ve all_entries'ye ekliyor: {len(self.rpymc_strings)} adet")
            rpymc_entries = []
            for s, path in zip(self.rpymc_strings, rpymc_files):
            ...
            # İlk 3 all_entries elemanını logla
            if 'all_entries' in locals():
                for i, entry in enumerate(all_entries[-len(rpymc_entries):][:3]):
                    self.log_message.emit('info', f"[DEBUG] all_entries son eklenen {i+1}: {entry}")
            ...
            for root, dirs, files in os.walk(rpy_dir):
                ...
                for filename in files:
                    ...
                    try:
                        ...
                        # UnicodeDecodeError'a karşı encoding fallback
                        try:
                            ...
                            try:
                                ...
                                self.log_message.emit('warning', f"{filename} dosyası encoding tespitine rağmen okunamadı: {enc} ({e})")
                                continue
