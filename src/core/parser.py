    ...
    def extract_text_entries(self, file_path: Union[str, Path]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for entry in py_entries:
                key = (entry.get('text', ''), entry.get('line_number', 0))
                if key not in seen_texts:
                    # context_path ve text_type zorunlu olsun
                    entry.setdefault('context_path', entry.get('context_line', ''))
            ...
            for descriptor in self.pattern_registry:
                ...
                for quote in quotes:
                    ...
                    if entry:
                        ...
                        if text_type in ["ui", "button", "translatable_string"]:
                            self.logger.info(f"[UI Extraction] {file_path}:{idx+1} [{text_type}] {text} (context: {current_context})")
                        if text_type == "python" or "_" in stripped_line:
                            self.logger.info(f"[PYTHON _() Extraction] {file_path}:{idx+1} {text}")
                break
        return entries
